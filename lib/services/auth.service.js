...
const blackListModelFunc = require('../models/black-list.model');
...
function authService(plugin, connection) {
    ...
    const BlackListModel = blackListModelFunc(connection, plugin);
    ...
    class AuthService {
        ...
        static async getUsers(app, query) {
            ...
            return await UserModel.find(filteredQuery, {
                __v: 0,
            }).select('-password -salt -userToken').exec();
        }

        static async getUserById(id) {
            return await UserModel.findById(id).select('-password -salt -userToken -__v').exec();
        }
        ...
        static async updateUser(id, data) {
            ...
            const token = await AuthService.createToken(userUpdate, true);
        ...
        static async updateUserMe(me, data) {
            ...
            const userToken = await AuthService.createToken(user, true);
            user.userToken = userToken;
            const userUpdate = await user.save();
            
            return UserTempSerializer.serialize(userUpdate).data;
        }
